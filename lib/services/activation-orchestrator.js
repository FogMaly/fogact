...
const { blue, boldGreen, color, cyan, gray, green, red, yellow } = require("../utils/colors");
...
function progressBar(percent, width = 20) {
  ...
  return `${green("█".repeat(filled))}${gray("░".repeat(width - filled))} ${percent}%`;
}

function printBanner() {
  console.log("");
  console.log(cyan("╭────────────────────────────────────────╮"));
  console.log(cyan("│") + "        FogAct 激活向导                 " + cyan("│"));
  console.log(cyan("│") + gray("    Claude Code / Codex 配置工具        ") + cyan("│"));
  console.log(cyan("╰────────────────────────────────────────╯"));
  console.log("");
}
...
function printCredentialProfile(service, upstream, apiKey, entitlement) {
  ...
  console.log("");
  console.log(gray("  API Key 信息"));
  console.log(gray(divider()));
  console.log(`${gray("  密钥:       ")}${cyan(maskKey(apiKey))}`);
  console.log(`${gray("  服务类型:   ")}${cyan(getServiceLabel(service))}`);
  console.log(`${gray("  状态:       ")}${green(raw.statusLabel || raw.status_label || raw.status || "正常")}`);
  console.log(`${gray("  计费模式:   ")}${raw.billingMode || raw.billing_mode || raw.billingType || raw.billing_type || raw.planName || entitlement.planName || "按额度计费"}`);
  console.log("");
  console.log(gray("  配额信息"));
  console.log(gray(divider()));
  console.log(`${gray("  每日配额:   ")}${cyan(formatQuotaValue(quota.daily, quota.unit))}`);
  console.log(`${gray("  今日已用:   ")}${yellow(formatQuotaValue(quota.used, quota.unit))}`);
  console.log(`${gray("  今日剩余:   ")}${green(formatQuotaValue(quota.remaining, quota.unit))}`);
  console.log(`${gray("  使用进度:   ")}${progressBar(quota.progress)}`);
  console.log("");
  console.log(gray("  有效期"));
  console.log(gray(divider()));
  console.log(`${gray("  激活时间:   ")}${formatDateTime(raw.activatedAt || raw.activated_at || raw.createdAt || raw.created_at)}`);
  console.log(`${gray("  到期时间:   ")}${formatDateTime(raw.expiresAt || raw.expires_at)}`);
  console.log(`${gray("  最后使用:   ")}${formatDateTime(raw.lastUsedAt || raw.last_used_at)}`);
  if (upstream.baseUrl) console.log(`${gray("  接入地址:   ")}${cyan(upstream.baseUrl)}`);
  console.log("");
}
...
function printResultSummary(service, backupPath, results, redeemResult) {
  ...
  console.log("");
  if (backupPath) {
    console.log(color("  ✓ 备份已创建", "green"));
  }

  const printConfigured = (entry, label, filePrefix = "配置") => {
    if (!entry) return false;
    if (entry.result.success) {
      console.log(boldGreen(`  ✓ ${label} 已激活`));
      const files = entry.result.files || [];
      if (files.length) console.log(`${color(`    ${filePrefix}: `, "gray")}${color(files.join(", "), "cyan")}`);
      return true;
    }
    if (!entry.result.skipped) {
      console.log(color(`  ✗ ${label} 激活失败: ${entry.result.error || entry.result.message || "未知错误"}`, "red"));
      return true;
    }
    return false;
  };
  ...
  if (!printConfigured(opencode, "OpenCode")) {
    console.log("");
    console.log(color("  ℹ 已跳过 OpenCode 配置（未检测到安装）", "gray"));
    console.log(color("    如需使用，请先运行一次 opencode 初始化后重新激活", "gray"));
  }

  const openclaw = byId.get("openclaw");
  if (!printConfigured(openclaw, "OpenClaw")) {
    console.log("");
    console.log(color("  ℹ 已跳过 OpenClaw 配置（未检测到安装）", "gray"));
    console.log(color("    如需使用，请先运行一次 openclaw 初始化后重新激活", "gray"));
  }
  ...
  if (extensionResults.length) {
    for (const entry of extensionResults) {
      if (entry.result.success) {
        console.log("");
        console.log(boldGreen(`  ✓ ${entry.platform.name} 已激活`));
        for (const file of entry.result.files || []) console.log(`${color("    目录: ", "gray")}${color(file, "cyan")}`);
      } else {
        console.log("");
        console.log(color(`  ⚠ ${entry.platform.name}: ${entry.result.error || entry.result.message || "无法激活"}`, "yellow"));
      }
    }
  } else if (service === "codex") {
    console.log("");
    console.log(color("  ℹ 已跳过编辑器插件配置（未检测到 Codex 插件）", "gray"));
  }

  console.log("");
  if (failed.length) {
    console.log(color(`  激活完成：${succeeded.length} 成功，${failed.length} 失败，${skipped.length} 跳过`, "yellow"));
  }
  if (service === "claude") {
    ...
    const message = tools.length === 1
      ? "  请重启 Claude Code 以应用新配置"
      : `  请重启相关工具（${tools.join("/")}）以应用新配置`;
    console.log(color(message, "gray"));
  } else {
    const tools = ["Codex", "VSCode", "Cursor"];
    if (byId.get("opencode")?.result.success) tools.push("OpenCode");
    if (byId.get("openclaw")?.result.success) tools.push("OpenClaw");
    console.log(color(`  请重启相关工具（${tools.join("/")}）以应用新配置`, "gray"));
  }
  console.log("");
}
...
function formatNodeChoice(result) {
  const latencyValue = result.avgLatency ?? result.latency ?? -1;
  const latency = result.available
    ? color(`${latencyValue}ms`, latencyValue < 60 ? "green" : latencyValue < 150 ? "yellow" : "red")
    : red("不可达");
...
async function selectAndVerifyNode(service = "codex", options = {}) {
  ...
  if (autoSelect) {
    console.log(`${green("✔")} ${blue("请选择节点:")} ${formatNodeChoice(selected)}${selected === best ? ` ${yellow("★ 推荐")}` : ""}`);
  }

  console.log("");
  console.log(gray("  正在验证节点..."));
  ...
  if (result.available || selected.available) {
    const latencyColor = latency < 60 ? "green" : latency < 150 ? "yellow" : "red";
    console.log(boldGreen(`  ✓ ${selected.name || "FogAct"} 已连接`));
    console.log(`${gray("    延迟: ")}${color(`${latency}ms`, latencyColor)}`);
    console.log(`${gray("    地址: ")}${cyan(selected.url)}`);
    console.log("");
    return { node: selected, latency };
  }

  console.log(red(`  ✗ ${selected.name || "FogAct"} 连接失败`));
  console.log(`${gray("    地址: ")}${cyan(selected.url)}`);
  console.log("");
  return null;
}
...
async function resolveCodeCredential(options, upstream) {
  ...
  if (!inspection.valid) {
    console.log(red(`✗ 无法读取激活码能力: ${inspection.error || "接口未返回有效信息"}`));
    return { cancelled: true };
  }
  ...
  if (!apiKey || !entitlementBaseUrl || entitlement.raw.proxy !== true) {
    console.log(red("✗ 激活接口没有返回 FogAct 中转配置，请联系管理员检查服务端代理设置。"));
    return { cancelled: true };
  }
  console.log(green("✔ 验证成功"));
  return {
    activationCode: code,
    apiKey,
    entitlement,
...
async function activateTargets({ service, upstream, apiKey, targets, activationCode, node = null, options = {} }) {
  const backupPath = createActivationBackup(service, getBackupPaths(targets), {
    ...
    nodeUrl: node?.node?.url || upstream.baseUrl || null,
...
async function runActivationWizard(options = {}) {
  ...
  if (!options.noNodeCheck) {
    selectedNode = await selectAndVerifyNode(options.service || "codex", options);
    if (!selectedNode) {
