...
const { color, shouldColorize } = require("../utils/colors");
...
function formatProbe(entry, colorEnabled) {
  const options = { color: colorEnabled };
  if (!entry?.ok) return color("--", "red", options);
  const value = `${entry.latency}ms`;
  return color(value, latencyColor(entry.latency), options);
}

function formatNodeLine(result, best, colorEnabled) {
  const options = { color: colorEnabled };
  const mark = color(result.available ? "✓" : "✗", result.available ? "green" : "red", options);
  ...
  if (!result.available) {
    return `  ${mark} ${name} ${ping} ${tcp} ${http}  ${color("不可达", "red", options)}`;
  }

  const latency = color(`${result.avgLatency}ms`, latencyColor(result.avgLatency), options);
  const bestMark = best && best === result ? ` ${color("★ 最优", "yellow", options)}` : "";
