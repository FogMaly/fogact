...
function enterFixedMenuScreen(stdout = process.stdout) {
  stdout.write(`\x1b[?1049h\x1b[?25l\x1b[?7l${ALT_SCROLL_OFF}`);
  clearScreen(stdout);
}
...
function leaveFixedMenuScreen(stdout = process.stdout) {
  ...
  clearScreen(stdout);
  stdout.write(`${ALT_SCROLL_ON}\x1b[?7h\x1b[?25h\x1b[?1049l`);
}
...
function isMouseInputToken(token) {
  return new RegExp(`^(?:${MOUSE_INPUT_PATTERN.source})$`, "i").test(String(token || ""));
}

function stripIgnoredReturnInput(input) {
  return String(input || "")
    .replace(MOUSE_INPUT_PATTERN, "")
    .replace(ARROW_INPUT_PATTERN, "");
}
...
function applyMenuInput(input, cursor, total = MENU_CHOICES.length) {
  ...
  const tokenPattern = new RegExp(`${MOUSE_INPUT_PATTERN.source}|\\u001b(?:\\[[0-9;]*[AB]|O[AB])|\\r|\\n|\\u0003|\\u001b|[1-9jksw]`, "gi");
  let match;
