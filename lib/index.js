...
const { cyan, gray } = require("./utils/colors");
...
const MOUSE_CAPTURE_ON = "\x1b[?1000h\x1b[?1002h\x1b[?1006h";
const MOUSE_CAPTURE_OFF = "\x1b[?1006l\x1b[?1002l\x1b[?1000l";
const ALT_SCROLL_OFF = "\x1b[?1007l";
const ALT_SCROLL_ON = "\x1b[?1007h";
const SCROLL_REGION_RESET = "\x1b[r";
const MOUSE_INPUT_PATTERN = /\u001b\[<\d+;\d+;\d+[mM]|\u001b\[\d+;\d+;\d+M|\u001b\[M[\s\S]{3}/gi;
const ARROW_INPUT_PATTERN = /\u001b(?:\[[0-9;]*[ABCD]|O[ABCD])/gi;
...
function renderMenu(cursor = 0) {
  ...
  const lines = [
    "",
    cyan("  ╭─────────────────────────────────────╮"),
    `${cyan("  │")}${padLine(title, 37)}${cyan("│")}`,
    `${cyan("  │")}${gray("    Claude Code / Codex 配置工具     ")}${cyan("│")}`,
    cyan("  ╰─────────────────────────────────────╯"),
    "",
    "? 请选择操作:",
  ];
  ...
  MENU_CHOICES.forEach((choice, index) => {
    ...
    const title = selected ? cyan(choice.title) : choice.title;
    lines.push(`${selected ? "❯" : " "} ${title}`);
  });

  lines.push("");
  lines.push(gray("↑↓ navigate • ⏎ select"));
  return lines.join("\n");
}
...
function enterFixedMenuScreen(stdout = process.stdout) {
  const rows = Number(stdout.rows || process.env.LINES || 40);
  const lockedScrollRegion = rows > 2 ? `\x1b[1;${rows - 1}r` : "";
  stdout.write(`${ALT_SCROLL_OFF}\x1b[?25l\x1b[?7l${lockedScrollRegion}`);
  enableMenuMouseCapture(stdout);
  clearScreen(stdout);
}

function leaveFixedMenuScreen(stdout = process.stdout) {
  disableMenuMouseCapture(stdout);
  clearScreen(stdout);
  stdout.write(`${SCROLL_REGION_RESET}${ALT_SCROLL_ON}\x1b[?7h\x1b[?25h`);
}

function enableMenuMouseCapture(stdout = process.stdout) {
  if (!stdout.isTTY) return;
  stdout.write(MOUSE_CAPTURE_ON);
}

function disableMenuMouseCapture(stdout = process.stdout) {
  if (!stdout.isTTY) return;
  stdout.write(MOUSE_CAPTURE_OFF);
}
...
function stripMouseInput(input) {
  return String(input || "").replace(MOUSE_INPUT_PATTERN, "");
}

function stripIgnoredReturnInput(input) {
  return stripMouseInput(input).replace(ARROW_INPUT_PATTERN, "");
}

function shouldReturnFromMenuWait(input) {
  const text = stripIgnoredReturnInput(input);
  return text.includes("\u0003") || text.includes("\r") || text.includes("\n");
}

function startIgnoredInputDrain(stdin = process.stdin, stdout = process.stdout) {
  ...
  const onData = (data) => {
    const remaining = stripIgnoredReturnInput(String(data));
...
async function withIgnoredInputDrain(callback, stdin = process.stdin, stdout = process.stdout) {
  const stopDrain = startIgnoredInputDrain(stdin, stdout);
  try {
...
function applyMenuInput(input, cursor, total = MENU_CHOICES.length) {
  const text = stripMouseInput(input);
...
function waitForMenuReturn(message = "按回车返回菜单...") {
  ...
  return new Promise((resolve) => {
    ...
    const cleanup = () => {
      ...
      if (stdin.isTTY && stdin.isRaw !== wasRaw) stdin.setRawMode(wasRaw);
      stdin.pause();
      disableMenuMouseCapture(process.stdout);
      process.stdout.write("\n");
      resolve();
    };

    const onData = (data) => {
      if (shouldReturnFromMenuWait(String(data))) {
        cleanup();
      }
    };
    ...
    stdin.setRawMode(true);
    stdin.resume();
    enableMenuMouseCapture(process.stdout);
    stdin.on("data", onData);
  });
}
    ...
    while (running) {
      ...
      switch (action) {
        case "activate":
          ...
          break;
        case "test":
          await withIgnoredInputDrain(() => runTestCommand());
...
module.exports = {
  ...
  startIgnoredInputDrain,
  stripIgnoredReturnInput,
  stripMouseInput,
  withIgnoredInputDrain,
  ensureLatestVersion,
  isLatestPackageRun,
  isNewerVersion,
  runCli,
